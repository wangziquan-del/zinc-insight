...
const html=read('index.html'),app=read('app.js'),dataText=read('data.json'),fail=[];
...
if(data){
  ...
  for(const chart of ['monthSpread','concentrateImport','refinedNetImport','galvanizedRateSeasonal','zincOxideRateSeasonal','dieCastRateSeasonal','galvanizedInventorySeasonal','globalExchangeStock','refinedTradeProfit'])ok((data.charts?.[chart]?.datasets||[]).some(x=>(x.data||[]).some(v=>v!=null)),`${chart} empty`);
  for(const chart of ['shfePrice','lmePrice','globalExchangeStock']){
  ...
  ok((data.charts?.refinedTradeProfit?.datasets||[]).length===2,'trade profit series != 2');
